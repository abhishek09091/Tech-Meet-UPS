...
public class userDetailDaoImpl implements userDetailDao{
	...
	@SuppressWarnings("unchecked")
	@Override
	public String attend(String email,String title,String date) {
		Session session=sf.getCurrentSession();
		System.out.println("bfr");
		Transaction tx=session.beginTransaction();
		UserDetail us=new UserDetail(email);
		
		Query result=session.createQuery("from MeetupDetail u where u.title='"+title+"'");
		System.out.println(result);
		
		List<MeetupDetail> list=(List<MeetupDetail>) result.list();
		for(MeetupDetail ms:list){
			
			System.out.println("ms id"+ms.getId());
